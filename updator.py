...
def convert_data(password):
    ...
    for index, line in enumerate(content):
        ...
        line = line.split(" | ")
        site, username, paswd = (
            decode(password, line[0]),
            decode(password, line[1]),
            decode(password, line[2].rstrip("\n")),
        )
        content[index] = (
            encrypt(program.get_key(), site + " | " + username + " | " + paswd) + "\n"
        )
    content[-1].rstrip("\n")
...
if __name__ == "__main__":
    print(
        ...
        + "Welcome to Updator."
        + Style.RESET_ALL
        + " Only use this if you want to convert your data from old MDPSaver version to the new one."
    )
    ...
    while True:
        ...
        try:
            ...
            print(
                Fore.CYAN
                + "Are you sure you have moved your old files into the new folders correctly?"
            )
            print(
                Fore.RED
                + "Update couldn't be completed. Try moving your old files again."
            )
            time.sleep(8)
            sys.exit()
        ...
        if hashing(password) == saved_password:
            print(
                Fore.MAGENTA
                + "It seems like it is already working. Update already done."
            )
    ...
    if updated == True and low_hash(password) == program.get_hashed_password():
        ...
        print(
            Fore.MAGENTA + "It seems like it is already working. Update already done."
        )
        print("You can use the program! Your password should be working!")
        sys.exit()
    elif updated == False:
        ...
        print(
            Fore.RED
            + "You need to reset your password for program to work (You can use the exact same info as before)"
        )
        program.change_access_password(True)
        print(Back.GREEN + "Program has successfully been updated!")
